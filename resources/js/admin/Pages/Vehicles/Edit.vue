  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">{{ vehicle.name || `${vehicle.brand} ${vehicle.model}` }}</p>
      </div>
      <Link
...
const submit = () => {
  console.log('🔵 Edit.vue - Soumission du formulaire', {
    vehicleId: props.vehicle.id,
    formData: form.data(),
    route: route('admin.vehicles.update', props.vehicle.id),
  });
  
  form.put(route('admin.vehicles.update', props.vehicle.id), {
    onSuccess: () => {
      console.log('✅ Edit.vue - Succès de la mise à jour');
      // Redirection gérée par le contrôleur
    },
    onError: (errors) => {
      console.error('❌ Edit.vue - Erreur lors de la mise à jour', errors);
    },
    onFinish: () => {
      console.log('🔵 Edit.vue - Soumission terminée');
    },
  });
};
...
const route = (name: string, params?: any): string => {
  const routes: Record<string, any> = {
    ...
    'admin.vehicles.update': (id: number) => `/admin/vehicles/${id}`,
