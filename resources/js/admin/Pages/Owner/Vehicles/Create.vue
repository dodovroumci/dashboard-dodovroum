<template>
  <div class="space-y-6">
    ...
    <div v-if="$page.props.flash?.success" class="bg-emerald-100 border border-emerald-400 text-emerald-700 px-4 py-3 rounded mb-4">
...
const handleFileUpload = async (event: Event) => {
  ...
  try {
    ...
    if (response.data.url || (response.data.success && response.data.url)) {
      if (!form.images) {
        form.images = [];
      }
...
const submit = () => {
  form.post(route('owner.vehicles.store'), {
    onSuccess: () => {
...
const errors = computed(() => {
  // Combiner les erreurs du formulaire et les erreurs du serveur
  return {
    ...form.errors,
    ...(page.props.errors || {}),
  };
...
const route = (name: string, params?: any): string => {
  const routes: Record<string, string> = {
    ...
    'owner.vehicles.store': '/owner/vehicles',
