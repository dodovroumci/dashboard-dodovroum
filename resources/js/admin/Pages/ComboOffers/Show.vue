    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 overflow-hidden">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between min-w-0">
        <div class="min-w-0 flex-1">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-slate-600 min-w-0">
            <div v-if="comboOffer?.description" class="text-slate-600 line-clamp-2 min-w-0">
              {{ comboOffer.description }}
            </div>
            <div v-if="comboOffer?.ownerName || comboOffer?.owner" class="flex items-center gap-2 min-w-0 truncate">
              <User class="w-4 h-4 shrink-0" />
              <span class="font-medium text-slate-700 truncate">
                {{ comboOffer?.ownerName || getOwnerName() }}
              </span>
            </div>
          </div>
        </div>
        <div class="flex items-center gap-2 relative shrink-0 flex-shrink-0">
          <Link
            :href="route('admin.combo-offers.edit', comboOffer?.id)"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
            <div class="min-w-0">
              ...
              <p class="font-medium text-slate-900 flex items-center gap-2 min-w-0 break-words">
                ...
                <span class="min-w-0 break-all">{{ comboOffer?.ownerName || getOwnerName() }}</span>
              </p>
            </div>
            <div class="min-w-0">
            ...
            <div class="min-w-0">
              ...
              <p class="font-medium text-slate-900 break-words">
                {{ formatDate(comboOffer?.startDate || comboOffer?.validFrom) }}
              </p>
            </div>
            <div class="min-w-0">
              <p class="text-sm text-slate-500 mb-1">Date de fin</p>
              <p class="font-medium text-slate-900 break-words">
                {{ formatDate(comboOffer?.endDate || comboOffer?.validTo) }}
              </p>
            </div>
          </div>
        ...
        <section class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 overflow-hidden">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4 min-w-0">
            ...
            <Link
              ...
              :href="route('admin.residences.show', comboOffer.residenceId)"
              class="text-sm text-blue-600 hover:text-blue-700 font-medium shrink-0"
            >
              Voir les détails →
            </Link>
        ...
        <section class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 overflow-hidden">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4 min-w-0">
            ...
            <Link
              ...
              :href="route('admin.vehicles.show', comboOffer.vehicleId)"
              class="text-sm text-blue-600 hover:text-blue-700 font-medium shrink-0"
            >
              Voir les détails →
            </Link>
        ...
        <div class="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all scale-100 border border-slate-100">
          <div class="mb-6 text-center">
            ...
            <p class="text-slate-500 mt-2">
              ...
              <strong class="text-slate-900">{{ comboOffer?.title || comboOffer?.name || 'cette offre' }}</strong> ?
          ...
          <div class="flex gap-3">
            ...
            >
              <span v-if="processing">Suppression...</span>
...
const deleteOffer = () => {
  if (!props.comboOffer?.id || processing.value) return;
  
  processing.value = true;
  
  router.delete(route('admin.combo-offers.destroy', props.comboOffer.id), {
    onBefore: () => {
      processing.value = true;
    },
    onSuccess: () => {
...
const getOwnerName = (): string => {
  if (props.comboOffer?.ownerName) {
    return props.comboOffer.ownerName;
  }
  if (props.comboOffer?.owner) {
    const owner = props.comboOffer.owner;
...
const route = (name: string, params?: any): string => {
  const routes: Record<string, any> = {
    ...
    'admin.combo-offers.edit': (id: string | number) => `/admin/combo-offers/${id}/edit`,
    'admin.combo-offers.destroy': (id: string | number) => `/admin/combo-offers/${id}`,
    'admin.residences.show': (id: string | number) => `/admin/residences/${id}`,
