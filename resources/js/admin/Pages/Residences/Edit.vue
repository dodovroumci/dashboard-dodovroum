...
// Fonction helper pour normaliser les images (décoder les chaînes JSON Prisma)
const normalizeImagesForForm = (rawImages: string | string[] | undefined): string[] => {
  if (!rawImages) return [];
...
// Fonction helper pour normaliser les amenities (décoder les chaînes JSON Prisma)
const normalizeAmenitiesForForm = (rawAmenities: string | string[] | undefined): string[] => {
  if (!rawAmenities) return [];
  if (Array.isArray(rawAmenities)) return rawAmenities;
  if (typeof rawAmenities === 'string') {
    try {
      ...
      console.error("Erreur de parsing des amenities:", error);
      return [];
    }
  }
  return [];
...
const submit = () => {
  form.put(route('admin.residences.update', props.residence.id), {
    onSuccess: () => {
...
const route = (name: string, params?: any): string => {
  const routes: Record<string, any> = {
    ...
    'admin.residences.update': (id: number) => `/admin/residences/${id}`,
